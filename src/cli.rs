...
pub fn main(opt: Options) -> Result<(), Error> {
    ...
    for facts_dir in &opt.fact_dirs {
        ...
        if opt.souffle {
            ...
            if opt.show_tuples {
                dump::dump_souffle_output(&output, &output_directory, tables)
        ...
        let (duration, output) = timed(|| Output::compute(&all_facts, algorithm, verbose || graphviz_output));
        if !opt.skip_timing {
            let seconds = duration.as_secs() as f64;
            let millis = f64::from(duration.subsec_nanos()) * 0.000_000_001_f64;
            println!("Time: {:0.3}s", seconds + millis);
...
FLAGS:
    -h, --help           Prints help information
        ...
        --souffle        Use Soufflé as a backend instead of Datafrog
